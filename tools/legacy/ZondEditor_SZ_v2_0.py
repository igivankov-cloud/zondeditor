...
def _rebuild_geo_from_template(original_bytes: bytes, blocks_info: list, prepared_tests: list) -> bytes:
    ...
    for i, t in enumerate(prepared_tests):
        ...
        n_pairs_max = max(0, data_len // 2)

        qc_in = list(getattr(t, 'qc', []) or [])
        fs_in = list(getattr(t, 'fs', []) or [])
        n_pairs = min(len(qc_in), len(fs_in), n_pairs_max)
...
class GeoCanvasEditor(tk.Tk):
        ...
        def _xview_proxy(*args):
            ...
            def _sync():
                ...
                try:
                    ...
                    n_tests = 0
                try:
                    col_w = float(self.w_depth + self.w_val + self.w_val)
                except Exception:
                    col_w = 0.0
                try:
    ...
    def load_and_render(self):
            ...
            # GEO/GE0 не содержит надёжных параметров глубины/шага — всегда считаем их неизвестными.
            self.depth_start = 0.0
            self.step_m = None
            self._depth_confirmed = False
            self._step_confirmed = False

            # Параметры глубины/шага в GEO отсутствуют.
    ...
    def _content_size(self):
        ...
        col_w = self.w_depth + self.w_val + self.w_val
    ...
    def _redraw(self):
        ...
        for col, ti in enumerate(self.display_cols):
            ...
            for r in range(max_rows):
                ...
                is_blank_row = (qc_txt.strip()=="" and fs_txt.strip()=="")
                ...
                def fill_for(kind: str):
                    ...
                    if data_i in getattr(fl, 'force_tail_rows', set()) and kind in ('depth','qc','fs'):
