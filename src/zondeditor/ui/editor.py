...
class GeoCanvasEditor(tk.Tk):
    ...
    def _geology_dirty_set(self) -> set[int | str]:
        dirty = self.__dict__.get("_geology_dirty_state", None)
        if dirty is None:
            dirty = {"__all__"}
            self.__dict__["_geology_dirty_state"] = dirty
        return dirty

    def _mark_geology_dirty(self, ti: int | None = None):
        dirty = self._geology_dirty_set()
        if ti is None:
            dirty.clear()
            dirty.add("__all__")
            return
        if "__all__" not in dirty:
            dirty.add(int(ti))

    def _clear_geology_dirty(self, ti: int | None = None):
        dirty = self._geology_dirty_set()
        if ti is None:
            dirty.clear()
            return
        dirty.discard(int(ti))

    def _is_geology_dirty(self, ti: int) -> bool:
        dirty = self._geology_dirty_set()
        return "__all__" in dirty or int(ti) in dirty

    def _clear_graph_layers(self, *, include_geology: bool = True):
        for ti in getattr(self, "display_cols", []) or []:
            card = self._card_for_test(int(ti))
            if card is not None and hasattr(card, "clear_body_render_layers"):
                tags = ["graph_axes", "graph_qc", "graph_fs", "graph_nodata"]
                if include_geology:
                    tags.extend(["layers_overlay", "layer_handles"])
                card.clear_body_render_layers(*tags)
    ...
    def _clear_card_graph_layers(self, ti: int, *, include_geology: bool = True):
        ti = int(ti)
        ...
        for cnv in (getattr(self, "canvas", None), getattr(self, "hcanvas", None)):
            ...
            if include_geology:
                tags.extend([f"layers_overlay_{ti}", f"layer_handles_{ti}"])
            for tag in tags:
                try:
                    cnv.delete(tag)
                except Exception:
    ...
    def _clear_card_geology_layers(self, ti: int):
        ti = int(ti)
        card = self._card_for_test(ti)
        if card is not None and hasattr(card, "clear_body_render_layers"):
        ...
        for cnv in (getattr(self, "canvas", None), getattr(self, "hcanvas", None)):
            if cnv is None:
                continue
            for tag in ("layers_overlay", f"layers_overlay_{ti}", "layer_handles", f"layer_handles_{ti}"):
                try:
                    cnv.delete(tag)
                except Exception:
                    pass
    ...
    def _drop_layer_hitboxes_for_test(self, ti: int):
        ti = int(ti)
        self._layer_handle_hitbox = [hit for hit in (self.__dict__.get("_layer_handle_hitbox", []) or []) if int(hit.get("ti", -1)) != ti]
        self._layer_depth_box_hitbox = [hit for hit in (self.__dict__.get("_layer_depth_box_hitbox", []) or []) if int(hit.get("ti", -1)) != ti]
        self._layer_plot_hitbox = [hit for hit in (self.__dict__.get("_layer_plot_hitbox", []) or []) if int(hit.get("ti", -1)) != ti]
        self._layer_label_hitbox = [hit for hit in (self.__dict__.get("_layer_label_hitbox", []) or []) if int(hit.get("ti", -1)) != ti]

    def _drop_all_layer_hitboxes(self):
        self._layer_handle_hitbox = []
        self._layer_depth_box_hitbox = []
        self._layer_plot_hitbox = []
        self._layer_label_hitbox = []
    ...
    def _refresh_card_graph_layers(self, ti: int):
        ...
        self._mark_geology_dirty(ti)
        self._render_card_graph_layers(ti)
        self._clear_geology_dirty(ti)
    ...
    def _header_render_payload(self, ti: int) -> dict | None:
        if ti < 0 or ti >= len(getattr(self, "tests", []) or []):
            return None
        card = self._card_for_test(int(ti))
        if card is None:
            return None
    ...
    def _refresh_card_header(self, ti: int):
        payload = self._header_render_payload(int(ti))
        if not payload:
            return
        card = payload.pop("card")
        header_canvas = getattr(card, "header_canvas", None) or self.hcanvas
        ...
        card.render_header(
            ...
            show_graph_scale=bool(self._is_graph_panel_visible()),
    ...
    def _render_card_graph_layers(self, ti: int, *, include_geology: bool = True):
        ...
        if show_geology:
            ...
            self._clear_geology_dirty(int(ti))
    ...
    def _redraw_graphs_now(self):
        ...
        if bool(self.__dict__.get("_scroll_in_progress", False)):
            ...
            return
        try:
            self._clear_graph_layers(include_geology=False)
        except TypeError:
            self._clear_graph_layers()
        if not self._is_graph_panel_visible():
            try:
                self._clear_graph_layers(include_geology=True)
            except TypeError:
                self._clear_graph_layers()
            self._drop_all_layer_hitboxes()
            self._clear_geology_dirty()
            return
        if not getattr(self, "tests", None):
            try:
                self._clear_graph_layers(include_geology=True)
        ...
        if not show_geology:
            for ti in self.display_cols:
                self._clear_card_geology_layers(int(ti))
            self._drop_all_layer_hitboxes()
            self._clear_geology_dirty()
        for ti in self.display_cols:
            ti = int(ti)
            include_geology = show_geology and self._is_geology_dirty(ti)
            if include_geology:
                self._clear_card_geology_layers(ti)
                self._drop_layer_hitboxes_for_test(ti)
            self._render_card_graph_layers(ti, include_geology=include_geology)
    ...
    def _on_left_click(self, event):
        ...
        if kind == "export":
            ...
            try:
                ...
                self._scan_by_algorithm(preview_mode=False)
                self._recompute_graph_scales()
                self._refresh_card_header(int(ti))
                self._render_card_graph_layers(int(ti))
                self._update_footer_realtime()
            except Exception:
                self._update_footer_realtime()
                self._redraw()
    ...
    def _set_hover(self, hover):
        if bool(getattr(self, "_inline_edit_active", False)):
            return
        prev_hover = getattr(self, "_hover", None)
        if prev_hover == hover:
            return
        self._hover = hover
        self._hide_canvas_tip()
        affected = set()
        for state in (prev_hover, hover):
            if isinstance(state, tuple) and len(state) >= 2 and state[0] in {"lock", "edit", "dup", "trash"}:
                try:
                    affected.add(int(state[1]))
                except Exception:
                    pass
        if not affected:
            return
