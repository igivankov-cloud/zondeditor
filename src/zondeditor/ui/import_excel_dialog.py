...
def _apply_row_bounds(rows: list[list[object]], ignored_rows: set[int], data_end_row: int) -> list[list[object]]:
    if not rows:
        return []
    bounded_end = max(1, int(data_end_row or len(rows)))
    out: list[list[object]] = []
    for idx, row in enumerate(rows, start=1):
        if idx in ignored_rows or idx > bounded_end:
...
class ExcelImportGrid(ttk.Frame):
    ...
    def _handle_click(self, event):
        ...
        if y <= (y_top + self.header_h) and x > self.row_header_w:
    ...
    def _redraw(self):
        ...
        y_top = self.canvas.canvasy(0)
        header_y0 = y_top
        header_y1 = y_top + self.header_h
        self.canvas.create_rectangle(0, header_y0, self.row_header_w, header_y1, fill="#f5f5f5", outline="#d0d0d0")
        ...
        for col in range(col_start, col_end):
            ...
            self.canvas.create_rectangle(cx, header_y0, cx + self.col_w, header_y1, fill=col_bg, outline="#d0d0d0")
        ...
        for row0 in range(row_start, row_end):
            ...
            if ridx == self.state.header_row:
                ...
                role = ROW_ROLE_DATA
            elif ridx == self.state.data_end_row:
                role = ROW_ROLE_DATA_END
...
class ExcelImportDialog(tk.Toplevel):
    ...
    def _reset_markup(self):
        ...
        self.header_row = 1
        self.data_start_row = 2
        self.data_end_row = max(1, len(self.current_sheet.rows))
    ...
    def _apply_detected_settings(self, detected: DetectedImportSettings):
        ...
        self.data_end_row = max(self.data_start_row, len(self.current_sheet.rows if self.current_sheet else []))
        self.ignored_rows = set()
    ...
    def _on_row_click(self, row_1based: int, x_root: int, y_root: int):
        ...
        menu.add_command(label="Последняя строка данных", command=lambda: self._set_row_role(row_1based, ROW_ROLE_DATA_END))
    ...
    def _set_row_role(self, row_1based: int, role: str):
        if role == ROW_ROLE_HEADER:
            ...
            if self.data_end_row < self.data_start_row:
                self.data_end_row = self.data_start_row
        elif role == ROW_ROLE_DATA_END:
            self.data_end_row = max(self.data_start_row, int(row_1based))
            if row_1based in self.ignored_rows:
                self.ignored_rows.discard(row_1based)
        elif role == ROW_ROLE_IGNORE:
    ...
    def _build_sheet_with_ignored_rows(self) -> WorkbookSheet | None:
        if not self.current_sheet:
            return None
        if not self.ignored_rows and int(self.data_end_row or 0) >= len(self.current_sheet.rows):
            return self.current_sheet
        rows = _apply_row_bounds(self.current_sheet.rows, set(self.ignored_rows), int(self.data_end_row or len(self.current_sheet.rows)))
