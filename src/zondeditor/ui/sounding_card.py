...
class SoundingCardGeometry:
    ...
    @property
    def header_info_band_local(self) -> tuple[float, float, float, float]:
        return (0.0, 0.0, float(self.table_width), float(self.header_height))

    @property
    def header_info_band_world(self) -> tuple[float, float, float, float]:
        lx0, ly0, lx1, ly1 = self.header_info_band_local
...
class SoundingCard:
    ...
    def render_header(
        ...
        *,
        ...
        sh_y = info_y1 - 8.0
        canvas.create_text(info_x0 + self.geometry.depth_width / 2, sh_y, text="H, м", font=("Segoe UI", 9), fill=header_text)
        canvas.create_text(info_x0 + self.geometry.depth_width + self.geometry.value_width / 2, sh_y, text="qc", font=("Segoe UI", 9), fill=header_text)
        canvas.create_text(info_x0 + self.geometry.depth_width + self.geometry.value_width + self.geometry.value_width / 2, sh_y, text="fs", font=("Segoe UI", 9), fill=header_text)
        ...
        if show_graph_scale and float(self.geometry.graph_width) > 0.0 and (scale_x1 - scale_x0) >= 40.0:
            ...
            if (panel_x1 - panel_x0) >= 40.0:
                ...
                panel_bottom = scale_y1 - 4.0
                split_y = (panel_top + panel_bottom) * 0.5
                qc_row = (panel_x0, panel_top, panel_x1, split_y - 2.0)
                fs_row = (panel_x0, split_y + 2.0, panel_x1, panel_bottom)
                ...
                qc_items += 1
                canvas.create_line(axis_left, qc_axis_y, axis_right, qc_axis_y, fill=qc_color, width=1)
                qc_items += 1
                for tick_x in (axis_left, (axis_left + axis_right) * 0.5, axis_right):
                    canvas.create_line(tick_x, qc_axis_y - 4.0, tick_x, qc_axis_y + 4.0, fill=qc_color, width=1)
                    qc_items += 1
                canvas.create_text(panel_x0 + 2.0, fs_row[1] + 1.0, text=fs_label, anchor="nw", font=("Segoe UI", 8), fill=fs_color)
                fs_items += 1
                canvas.create_line(axis_left, fs_axis_y, axis_right, fs_axis_y, fill=fs_color, width=1)
                fs_items += 1
                for tick_x in (axis_left, (axis_left + axis_right) * 0.5, axis_right):
                    canvas.create_line(tick_x, fs_axis_y - 4.0, tick_x, fs_axis_y + 4.0, fill=fs_color, width=1)
                    fs_items += 1
                if debug_enabled:
                    try:
                        print(
