...
def iter_builtin_preview_patterns() -> list[HatchPattern]:
    patterns: list[HatchPattern] = []
    for soil in PREVIEW_SOILS:
        pattern = load_registered_hatch(soil)
        if pattern is not None:
            patterns.append(pattern)
    return patterns


def draw_hatch_preview_grid(canvas: Any, *, x0: float = 0.0, y0: float = 0.0, cell_w: float = 180.0, cell_h: float = 92.0, columns: int = 2, padding: float = 10.0, patterns: Iterable[HatchPattern] | None = None, tags: Any = ('hatch_preview',)) -> tuple[float, float, float, float]:
    ...
    for idx, pattern in enumerate(pats):
        ...
        render_hatch_pattern(canvas, (bx0 + 6, by0 + 24, bx1 - 6, by1 - 6), pattern, tags=tags, scale_info={'usage': HATCH_USAGE_EDITOR_COLLAPSED, 'layer_height_px': float(max(1.0, (by1 - by0) - 30.0)), 'logical_rect': (bx0 + 6, by0 + 24, bx1 - 6, by1 - 6)})
